...
----------------------------------------
-- Basic progression states:
-- Dead + 3 neighbors = alive
-- Alive + neighbors < 2 = dead
-- Alive + neighbors > 3 = dead
-- Alive + 3 >= neighbors >= 2 = Alive
----------------------------------------

-- | Algorithm implementation
nextGeneration :: World -> World
nextGeneration w = S.union (survivingCells w) (bornCells w)

survivingCells :: World -> World
survivingCells w = S.filter (\cell -> let count = neighborCountIn cell w in count == 2 || count == 3) w

bornCells :: World -> World
bornCells w = S.fromList [cell | cell <- S.toList (deadCells w), neighborCountIn cell w == 3]

deadCells :: World -> World
deadCells w = S.fromList [cell | cell <- S.toList (allPotentialCells w), not (cell `S.member` w)]
  where allPotentialCells w = S.unions [neighbors cell | cell <- S.toList w]
